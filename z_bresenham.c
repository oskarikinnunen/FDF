...
/*   Updated: 2022/03/23 00:55:49 by okinnune         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

void	step_z_bresenham_x(t_bresenham *b, int target[3])
{
	if (b->error * 2 <= b->diff[X] && b->local[Y] != target[Y])
		b->error += (b->local[Y] += b->add[Y], b->diff[X]);
}

void	step_z_bresenham_y(t_bresenham *b, int target[3])
{
	if (b->error * 2 >= b->diff[Y] && b->local[X] != target[X])
		b->error += (b->local[X] += b->add[X], b->diff[Y]);
}

void	populate_bresenham3d(t_bresenham *b, int *from, int *to)
{
